...
def word_count(filename):
  ...
  with open(filename, "r") as file:
    ...
    for word in words:
      if word.lower() not in wordCounter:
...
def print_words(filename):
  wordCounter = word_count(filename)
  result = sorted(wordCounter.items())
  for word in result:
    print(word[0], word[1])

def print_top(filename):
  wordCounter = word_count(filename)
  result = sorted(wordCounter.items(), key=lambda item: item[1], reverse=True)
  for word in result:
    print(word[0], word[1])
